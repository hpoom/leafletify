...
( function( $, window, document, undefined ) {
	...
	$.fn.leafletify = function( options ) {
		try {
			...
			_debug = settings.debug ? _debug : function() {};
			...
			var tileLayer = localL.tileLayer( 'http://{s}.tile.osm.org/{z}/{x}/{y}.png', {
			...
			this.each( function() {
				try {
					...
					if ( latitude.length && longitude.length ) {
						...
						if ( mapPoints[data.mapid] === undefined ) {
							mapPoints[data.mapid] = [];
						}
			...
			for ( var mapId in mapPoints ) {
				try {
					...
					if ( $mapContainer.length ) {
						...
						$mapContainer.data( 'points', mapPoints[mapId] );
						...
						$mapContainer.on( 'showMap', function() {
							...
							if ( mapInitialized === true ) {
								...
								// with multiple maps.
								var tiles = jQuery.extend( {}, tileLayer );
								tiles.addTo( map ); // Add this layer to our map

								// Loop over the points
								for ( var pointCount = 0, len = mapPoints[mapId].length; pointCount < len; pointCount++ ) {
									// Catch any problems & move on to next marker here
									try {
										// Hold this points lat & lon
										var latLng = [mapPoints[mapId][pointCount].lat,mapPoints[mapId][pointCount].lon];
										...
										if ( mapPoints[mapId][pointCount].icondiv ) {
											markerOptions.icon = mapPoints[mapId][pointCount].icondiv;
										}
										...
										if ( mapPoints[mapId][pointCount].popover !== undefined ) {
											marker.bindPopup( mapPoints[mapId][pointCount].popover );
										}
									} catch( e ) {
